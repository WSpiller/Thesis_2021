...
library(lme4)
library(ggplot2)
data<-read.csv("cc_data.csv",header=T)
...
data$sbp1<-scale(data$sbp1)
data$dbp0<-scale(data$dbp0)
data$dbp1<-scale(data$dbp1)
...
stroke_cc_res2<-stroke_cc_res[c(2,6:10,3,5,4,11,12),]

stroke_cc_res2

res_data6<-data.frame(c(stroke_cc_res2[,1]
...
names(res_data6)<-c("effect","lci_95","uci_95","exposure","index")

res_data6$index<-res_data6$index * -1

uniplot6<-ggplot(res_data6)+geom_point(aes(x=index, y=effect))+
  coord_flip()+geom_errorbar(aes(ymin=lci_95,ymax=uci_95,x=index),width=0,orientation = "x")+
  scale_y_continuous(limits = c(min(res_data6$lci_95),max(res_data6$uci_95)),name = "Effect (OR)")+geom_hline(yintercept=1,linetype = "dashed")+theme_bw()+
  scale_x_continuous(limits = c(-11,-1),breaks=res_data6$index,labels= res_data6$exposure,name = "")+
...
png("CC_Forest1.png",width=5.33,height=4,units="in",res=500)
uniplot6
dev.off()
